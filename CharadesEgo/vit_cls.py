...
class ViTCls(nn.Module):
    def __init__(self,dim, depth, heads, mlp_dim, pool = 'cls', dim_head = 64, dropout = 0., emb_dropout = 0., num_classes=157):
        super().__init__()
    ...
    def forward(self, slow_x, fast_x, audio_feat, target=False, another_slow_x=None, another_fast_x=None, another_audio_feat = None):
        if another_slow_x is not None:
            v_feat = self.to_patch_embedding2(slow_x)
            v_feat2 = self.to_patch_embedding3(fast_x)
            v_feat = torch.cat((v_feat, v_feat2), dim=1)

            another_slow_x = self.to_patch_embedding2(another_slow_x)
            another_fast_x = self.to_patch_embedding3(another_fast_x)
            another_v_feat = torch.cat((another_slow_x, another_fast_x), dim=1)
            ...
            x1 = torch.cat((judge_token, v_feat, audio_feat), dim=1)
            x2 = torch.cat((judge_token, another_v_feat, another_audio_feat), dim=1)
            ...
            #print(slow_x.size(), self.to_patch_embedding2)
            v_feat = self.to_patch_embedding2(slow_x)
            v_feat2 = self.to_patch_embedding3(fast_x)
            v_feat = torch.cat((v_feat, v_feat2), dim=1)
            #print(v_feat.size())
            ...
            x = torch.cat((cls_tokens,v_feat, audio_feat), dim=1)
