...
if __name__ == '__main__':
    ...
    checkpoint = torch.load(
        "checkpoints/best_%s2%s_CharadesEgo_encoder.pt" % (args.source_domain, args.target_domain, args.save_id))
    ...
    log_path = base_path + "log%s2%s_encoder_finetuned.csv" % (args.source_domain, args.target_domain,)
    cmd = ['rm -rf ', log_path]
    os.system(' '.join(cmd))
    ...
    with open(log_path, "a") as f:
        for epoch_i in range(3):
            ...
            if map > BestMAP:
                ...
                torch.save(save,
                           base_path + "best_%s2%s_CharadesEgo_encoder_finetuned.pt" % (
