...
from vit import ViT
from dataloader_encoder import EPICDOMAINClusters, EPICDOMAIN
...
def train_one_step(model, attention_model, clip, spectrogram, labels, target_label, weights):
    ...
    channel_att = attention_model(audio_feat.detach())
    ...
    target_channel_att = attention_model(target_audio_feat.detach())
    adapted_target_feat = torch.sigmoid(
       target_channel_att.unsqueeze(2).unsqueeze(2).unsqueeze(2)) * target_v_feat.detach()
...
if __name__ == '__main__':
    ...
    config_file = 'configs/recognition/slowonly/slowonly_r50_8x8x1_256e_kinetics400_flow.py'
    ...
    optim = torch.optim.Adam(list(model.module.backbone.layer4.parameters())+list(model.module.cls_head.parameters()) + list(attention_model.parameters()), lr=opts.lr_1stStage, weight_decay=1e-4)
