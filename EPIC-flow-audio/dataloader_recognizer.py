...
class EPICDOMAINRecognizer(torch.utils.data.Dataset):
    def __init__(self, split='train', source_domain='D1', target_domain='D2', modality='rgb', cfg=None, use_audio=True):
        ...
        target_train_file = pd.read_pickle('/home/yzhang8/data/EPIC_KITCHENS_UDA/'+target_domain+"_"+split+".pkl")
        ...
        for _, line in target_train_file.iterrows():
            image = [target_domain + '/' + line['video_id'], line['start_frame'], line['stop_frame'],
                     ...
                     line['stop_timestamp']]
            # labels = line['verb_class']
            tmp = int(np.ceil(line['start_frame'] / 2))
    ...
    def __getitem__(self, index):
        video_path = self.base_path +'frames_rgb_flow/flow/'+self.split + '/'+self.source_samples[index][0]
        index2 = self.target_idx_list.pop(0)
        self.target_idx_list.append(index2)
        target_video_path = self.base_path +'frames_rgb_flow/flow/'+self.split + '/'+self.target_samples[index2][0]

        filename_tmpl = self.cfg.data.train.get('filename_tmpl', 'frame_{:010}.jpg')
        modality = self.cfg.data.train.get('modality', 'Flow')
        start_index = self.cfg.data.train.get('start_index', int(np.ceil(self.source_samples[index][1] / 2)))
        data = dict(
            frame_dir=video_path,
            total_frames=int((self.source_samples[index][2] - self.source_samples[index][1])/2),
        ...
        start_index = self.cfg.data.train.get('start_index', int(np.ceil(self.target_samples[index2][1] / 2)))
        target_data = dict(
            frame_dir=target_video_path,
            total_frames=int((self.target_samples[index2][2] - self.target_samples[index2][1]) / 2),
            # assuming files in ``video_path`` are all named with ``filename_tmpl``  # noqa: E501
            label=-1,
            start_index=start_index,
            filename_tmpl=filename_tmpl,
...
class EPICDOMAIN(torch.utils.data.Dataset):
    def __init__(self, split='train', domain='D1', modality='rgb', cfg=None, use_audio=True):
        self.base_path = '/home/yzhang8/data/EPIC_KITCHENS_UDA/'
        train_file = pd.read_pickle('/home/yzhang8/data/EPIC_KITCHENS_UDA/'+domain+"_"+split+".pkl")
        self.domain = domain
        self.split = split
        self.modality = modality
        self.use_audio = use_audio
    ...
    def __getitem__(self, index):
        video_path = self.base_path +'frames_rgb_flow/flow/'+self.split + '/'+self.samples[index][0]
        if self.split == 'train':
            filename_tmpl = self.cfg.data.train.get('filename_tmpl', 'frame_{:010}.jpg')
            modality = self.cfg.data.train.get('modality', 'Flow')
            start_index = self.cfg.data.train.get('start_index', int(np.ceil(self.samples[index][1] / 2)))
            data = dict(
                frame_dir=video_path,
            ...
            modality = self.cfg.data.val.get('modality', 'Flow')
            start_index = self.cfg.data.val.get('start_index', int(np.ceil(self.samples[index][1] /2)))
            data = dict(
                frame_dir=video_path,
                total_frames=int((self.samples[index][2] - self.samples[index][1])/2),
                # assuming files in ``video_path`` are all named with ``filename_tmpl``  # noqa: E501
